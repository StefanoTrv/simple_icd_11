...
class TestICDOtherAPIClient(unittest.TestCase):
    ...
    def testLookupCodeOk(self):
        json_dict = self.client.lookupCode("1F0Y","2025-01","en")
        self.assertEqual(json_dict["code"],"1F0Y")
        self.assertEqual(json_dict["@id"],"http://id.who.int/icd/release/11/2025-01/mms/1646490591/other")
    
    def testLookupCodeNotExists(self):
        with self.assertRaises(LookupError):
            self.client.lookupCode("banana","2025-01","en")
    
    def testLookupIdOk(self):
        json_dict = self.client.lookupId("218513628","2025-01","en")
        self.assertEqual(json_dict["code"],"9B71.1")
        self.assertEqual(json_dict["@id"],"http://id.who.int/icd/release/11/2025-01/mms/218513628")
    
    def testLookupIdNotExists(self):
        with self.assertRaises(LookupError):
            self.client.lookupId("5","2025-01","en")
    ...
    def testCheckReleaseTrue(self):
        self.assertTrue(self.client.checkRelease("2025-01","en"))
    ...
    def testGetBrowserUrl(self):
        explorer = ICDExplorer("en","","",release="2025-01",customUrl=url)
        e = explorer.getEntityFromCode("V")
        self.assertEqual(e.getBrowserUrl(),"http://localhost/browse/2025-01/mms/en#231358748")
        e = explorer.getEntityFromId("2091156678")
        self.assertEqual(e.getBrowserUrl(),"http://localhost/browse/2025-01/mms/en#2091156678")
...
class TestProxyEntity(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.explorer = ICDExplorer("en","","",release="2025-01",customUrl=url)
    ...
    def testDoesLookup(self):
        prx = ProxyEntity(self.explorer,"557175275","icd.who.int/browse/2025-01/mms/557175275")
